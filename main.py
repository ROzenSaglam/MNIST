...
def run_softmax_on_MNIST_pca(temp_parameter=1):
    ...
    return test_error
print("test_error_18_comp_PCA:", run_softmax_on_MNIST_pca(temp_parameter=1))
...
# train_pca (and test_pca) is a representation of our training (and test) data
# after projecting each example onto the first 10 principal components.
...
def run_softmax_on_MNIST_pca(temp_parameter=1):
    ...
    plot_cost_function_over_time(cost_function_history)
    # Save the model parüameters theta obtained from calling softmax_regression to disk.
    write_pickle_data(theta, "./theta.pkl.gz")
    test_error = compute_test_error(test_cube, test_y, theta, temp_parameter)
    return test_error
print("test_error_10_comp_PCA:", run_softmax_on_MNIST_pca(temp_parameter=1))

def poly_svm(train_pca10,  train_y, test_pca10, test_y):
    """
    Trains a linear SVM for multiclass classifciation using a one-vs-rest strategy

    Args:
        train_pca10 - 10-dimensional PCA representation of the training NumPy array (n datapoints each with 10 features)
        train_y - NumPy array containing the labels (int) for each training data point
        test_pca10 - 10-dimensional PCA representation of the test set  NumPy array (m datapoints each with 10 features)
        test_y - NumPy array containing the labels (int) for each test data point
    Returns:
        pred_test_y - NumPy array containing the labels (int) for each test data point
    """
    svc = SVC(random_state= 0, kernel='rbf')
    svc.fit(train_pca10,train_y)
    pred_test_y = svc.predict(test_pca10)
    return 1 - np.mean(pred_test_y == test_y)
    raise NotImplementedError

print("Polynomial svm test error =", poly_svm(train_pca10,  train_y, test_pca10, test_y))
